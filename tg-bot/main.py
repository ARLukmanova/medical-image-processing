...
async def send_results(message: Message, result: Dict[str, Any], image_bytes: bytes):
    ...
    try:
        ...
        result_text = (
            ...
            f"<i>Использована гибридная модель (ResNet + EfficientNet)</i>\n"
...
async def interpreting_help(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        ...
        "Гибридная модель сочетает преимущества ResNet и EfficientNet для более точной диагностики.",
        parse_mode=ParseMode.HTML
    )
...
async def cmd_start(message: Message):
    ...
    try:
        await message.answer(
            ...
            "Используется гибридная модель (ResNet + EfficientNet) для максимальной точности."
        )
    except Exception as e:
